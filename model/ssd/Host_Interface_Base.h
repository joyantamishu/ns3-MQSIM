...
{
#define COPYDATA(DEST,SRC,SIZE) if(MQsim_Simulator->Is_integrated_execution_mode()) {DEST = new char[SIZE]; memcpy(DEST, SRC, SIZE);} else DEST = SRC;
#define DELETE_REQUEST_NVME(REQ) \
	delete (Submission_Queue_Entry*)REQ->IO_command_info; \
	if(MQsim_Simulator->Is_integrated_execution_mode())\

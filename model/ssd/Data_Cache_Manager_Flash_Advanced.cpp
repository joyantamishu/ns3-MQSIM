...
namespace SSD_Components
{
	...
	{
		//double total_write_arrival_rate = 0,
		//double total_read_arrival_rate = 0;
		switch (sharing_mode) {
	...
	void Data_Cache_Manager_Flash_Advanced::write_to_destage_buffer(User_Request* user_request)
	{
		...
		if (MQsim_Simulator->Time() > next_bloom_filter_reset_milestone) {
			bloom_filter[user_request->Stream_id].clear();
			next_bloom_filter_reset_milestone = MQsim_Simulator->Time() + bloom_filter_reset_step;
		}
	}
	...
	void Data_Cache_Manager_Flash_Advanced::service_dram_access_request(Memory_Transfer_Info* request_info)
	{
		if (memory_channel_is_busy) {
			...
			MQsim_Simulator->Register_sim_event(MQsim_Simulator->Time() + estimate_dram_access_time(request_info->Size_in_bytes, dram_row_size,
	...
	void Data_Cache_Manager_Flash_Advanced::Execute_MQsim_Simulator_event(MQSimEngine::Sim_Event* ev)
	{
		...
		if (shared_dram_request_queue)	{
			if (dram_execution_queue[0].size() > 0)	{
				...
				dram_execution_queue[0].pop();
				MQsim_Simulator->Register_sim_event(MQsim_Simulator->Time() + estimate_dram_access_time(transfer_info->Size_in_bytes, dram_row_size, dram_busrt_size,

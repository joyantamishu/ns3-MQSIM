...
namespace ns3
{
	...
	void TSU_Priority_OutOfOrder::Execute_MQsim_Simulator_event(MQSimEngine::Sim_Event *event)
	{
	}
	...
	bool TSU_Priority_OutOfOrder::service_read_transaction(NVM::FlashMemory::Flash_Chip *chip)
	{
		...
		case ChipStatus::WRITING:
			...
			if (_NVMController->Expected_finish_time(chip) - MQsim_Simulator->Time() < writeReasonableSuspensionTimeForRead)
			{
				return false;
			}
		...
		case ChipStatus::ERASING:
			...
			if (_NVMController->Expected_finish_time(chip) - MQsim_Simulator->Time() < eraseReasonableSuspensionTimeForRead)
			{
				return false;
			}
			suspensionRequired = true;
	...
	bool TSU_Priority_OutOfOrder::service_write_transaction(NVM::FlashMemory::Flash_Chip *chip)
	{
		...
		case ChipStatus::ERASING:
			if (!eraseSuspensionEnabled || _NVMController->HasSuspendedCommand(chip))
				return false;
			if (_NVMController->Expected_finish_time(chip) - MQsim_Simulator->Time() < eraseReasonableSuspensionTimeForWrite)
				return false;
			suspensionRequired = true;
		default:
			return false;

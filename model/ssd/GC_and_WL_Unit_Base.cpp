#include "ns3/GC_and_WL_Unit_Base.h"
...
namespace SSD_Components
{
		...
		switch (transaction->Type) {
			...
			case Transaction_Type::WRITE:
				if (pbke->Blocks[((NVM_Transaction_Flash_WR*)transaction)->RelatedErase->Address.BlockID].Holds_mapping_data) {
					...
					DEBUG(MQsim_Simulator->Time() << ": MVPN=" << (MVPN_type)transaction->LPA << " unlocked!!");
				} else {
					_my_instance->address_mapping_unit->Remove_barrier_for_accessing_lpa(transaction->Stream_id, transaction->LPA);
					DEBUG(MQsim_Simulator->Time() << ": LPA=" << (MVPN_type)transaction->LPA << " unlocked!!");
				}
	...
	void GC_and_WL_Unit_Base::Execute_MQsim_Simulator_event(MQSimEngine::Sim_Event* ev)
	{
	}

...
namespace SSD_Components
{
	...
	void Queue_Probe::EnqueueRequest(NVM_Transaction* transaction)
	{
		...
		currentObjectsInQueue[transaction] = MQsim_Simulator->Time();
		nRequests++;
		nRequestsEpoch++;
		setCount(count + 1);
	}
	...
	void Queue_Probe::DequeueRequest(NVM_Transaction* transaction)
	{
		...
		sim_time_type tc = MQsim_Simulator->Time() - et;
		totalWaitingTime += tc;
	...
	{
		sim_time_type n = MQsim_Simulator->Time();
	...
	void Queue_Probe::ResetEpochStatistics()
	{
		...
		epochStartTime = MQsim_Simulator->Time();
	...
	void Queue_Probe::Snapshot(std::string id, Utils::XmlWriter& writer)
	{
		...
		for (unsigned int i = 0; i < states.size(); i++) {
			...
			sim_time_type now = MQsim_Simulator->Time();
	...
	double Queue_Probe::AvgQueueLength()
	{
		...
		return (double)sum / (double)MQsim_Simulator->Time();
	}
	...
	double Queue_Probe::STDevQueueLength()
	{
		...
		double mean = (double)sum / (double)MQsim_Simulator->Time();
		double stdev = 0.0;
		for (unsigned int len = 0; len < states.size(); len++) {
			stdev += std::pow((double)states[len].totalTime * len / (double)MQsim_Simulator->Time() - mean, 2);
		}
	...
	double Queue_Probe::AvgQueueLengthEpoch()
	{
		...
		return (double)sum / (double)(MQsim_Simulator->Time() - epochStartTime);
	}

...
namespace SSD_Components
{
	...
	{
		MQsim_Simulator->Register_sim_event(flow_classification_epoch, this, 0, 0);
	}

	void TSU_FLIN::Validate_simulation_config() {}

	void TSU_FLIN::Execute_MQsim_Simulator_event(MQSimEngine::Sim_Event* event)
	{
		...
		MQsim_Simulator->Register_sim_event(MQsim_Simulator->Time() + flow_classification_epoch, this, 0, 0);
	}
	...
	void TSU_FLIN::reorder_for_fairness(Flash_Transaction_Queue* queue, std::list<NVM_Transaction_Flash*>::iterator start, std::list<NVM_Transaction_Flash*>::iterator end)
	{
		...
		if(_NVMController->Is_chip_busy(*itr))
			if (_NVMController->Expected_finish_time(*itr) > MQsim_Simulator->Time())
				time_to_finish = _NVMController->Expected_finish_time(*itr) - MQsim_Simulator->Time();//T^chip_busy
		...
		while (itr != std::next(end))
		{
			...
			time_to_finish += _NVMController->Expected_transfer_time(*itr) + _NVMController->Expected_finish_time(*itr);
			sim_time_type T_TR_shared = time_to_finish + (MQsim_Simulator->Time() - (*itr)->Issue_time);
	...
	void TSU_FLIN::estimate_alone_waiting_time(Flash_Transaction_Queue* queue, std::list<NVM_Transaction_Flash*>::iterator position)
	{
		...
		if (itr == queue->begin() && (*itr)->Stream_id != (*position)->Stream_id)
		{
			...
			else
			{
				...
				if (expected_last_time > MQsim_Simulator->Time())
					(*position)->Estimated_alone_waiting_time = expected_last_time - MQsim_Simulator->Time();

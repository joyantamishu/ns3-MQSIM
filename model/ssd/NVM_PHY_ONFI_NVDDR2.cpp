...
#include "ns3/NVM_PHY_ONFI_NVDDR2.h"
#include "ns3/Stats.h"
...
namespace SSD_Components {
		...
		switch (transaction_list.front()->Type) {
			case Transaction_Type::READ:
				...
				if (chipBKE->OngoingDieCMDTransfers.size() == 0) {
					...
					chipBKE->Last_transfer_finish_time = MQsim_Simulator->Time() + suspendTime + target_channel->ReadCommandTime[transaction_list.size()];
					MQsim_Simulator->Register_sim_event(MQsim_Simulator->Time() + suspendTime + target_channel->ReadCommandTime[transaction_list.size()], this,
			...
			case Transaction_Type::WRITE:
				if (((NVM_Transaction_Flash_WR*)transaction_list.front())->ExecutionMode == WriteExecutionModeType::SIMPLE) {
					...
					if (chipBKE->OngoingDieCMDTransfers.size() == 0) {
						...
						chipBKE->Last_transfer_finish_time = MQsim_Simulator->Time() + suspendTime + target_channel->ProgramCommandTime[transaction_list.size()] + data_transfer_time;
						MQsim_Simulator->Register_sim_event(MQsim_Simulator->Time() + suspendTime + target_channel->ProgramCommandTime[transaction_list.size()] + data_transfer_time,
			...
			case Transaction_Type::ERASE:
				...
				if (chipBKE->OngoingDieCMDTransfers.size() == 0) {
					targetChip->StartCMDXfer();
					chipBKE->Status = ChipStatus::CMD_IN;
					chipBKE->Last_transfer_finish_time = MQsim_Simulator->Time() + suspendTime + target_channel->EraseCommandTime[transaction_list.size()];
					MQsim_Simulator->Register_sim_event(MQsim_Simulator->Time() + suspendTime + target_channel->EraseCommandTime[transaction_list.size()],
	...
	void NVM_PHY_ONFI_NVDDR2::Execute_MQsim_Simulator_event(MQSimEngine::Sim_Event* ev)
	{
		...
		switch ((NVDDR2_SimEventType)ev->Type) {
			case NVDDR2_SimEventType::READ_CMD_ADDR_TRANSFERRED:
				...
				for (auto tr : dieBKE->ActiveTransactions) {
					tr->STAT_execution_time = dieBKE->Expected_finish_time - MQsim_Simulator->Time();
				}
		...
		if (WaitingCopybackWrites[channel_id].size() > 0) {
			...
			MQsim_Simulator->Register_sim_event(MQsim_Simulator->Time() + this->channels[channel_id]->ProgramCommandTime[waitingBKE->ActiveTransactions.size()],
			...
			waitingBKE->Expected_finish_time = MQsim_Simulator->Time() + this->channels[channel_id]->ProgramCommandTime[waitingBKE->ActiveTransactions.size()]
	...
	inline void NVM_PHY_ONFI_NVDDR2::handle_ready_signal_from_chip(NVM::FlashMemory::Flash_Chip* chip, NVM::FlashMemory::Flash_Command* command)
	{
		...
		case CMD_READ_PAGE_COPYBACK_MULTIPLANE:
			...
			if (_my_instance->channels[chip->ChannelID]->GetStatus() == BusChannelStatus::IDLE)
			{
				...
				chip->StartCMDXfer();
				chipBKE->Status = ChipStatus::CMD_IN;
				MQsim_Simulator->Register_sim_event(MQsim_Simulator->Time() + _my_instance->channels[chip->ChannelID]->ProgramCommandTime[dieBKE->ActiveTransactions.size()],
				...
				dieBKE->Expected_finish_time = MQsim_Simulator->Time() + _my_instance->channels[chip->ChannelID]->ProgramCommandTime[dieBKE->ActiveTransactions.size()]
				...
				MQsim_Simulator->RegisterEvent(MQsim_Simulator->Time() + channels[targetChip->ChannelID]->ProgramCommandTime + NVDDR2DataOutTransferTime(targetTransaction->SizeInByte, channels[targetChip->ChannelID]),
	...
	inline void NVM_PHY_ONFI_NVDDR2::transfer_read_data_from_chip(ChipBookKeepingEntry* chipBKE, DieBookKeepingEntry* dieBKE, NVM_Transaction_Flash* tr)
	{
		...
		MQsim_Simulator->Register_sim_event(MQsim_Simulator->Time() + NVDDR2DataOutTransferTime(tr->Data_and_metadata_size_in_byte, channels[tr->Address.ChannelID]),
	...
	void NVM_PHY_ONFI_NVDDR2::perform_interleaved_cmd_data_transfer(NVM::FlashMemory::Flash_Chip* chip, DieBookKeepingEntry* bookKeepingEntry)
	{
		...
		switch (bookKeepingEntry->ActiveTransactions.front()->Type)
		{
			case Transaction_Type::READ:
				...
				MQsim_Simulator->Register_sim_event(MQsim_Simulator->Time() + bookKeepingEntry->DieInterleavedTime,
			...
			case Transaction_Type::WRITE:
				if (((NVM_Transaction_Flash_WR*)bookKeepingEntry->ActiveTransactions.front())->RelatedRead == NULL) {
					...
					chip->StartCMDXfer();
					bookKeepingTable[chip->ChannelID][chip->ChipID].Status = ChipStatus::CMD_IN;
					MQsim_Simulator->Register_sim_event(MQsim_Simulator->Time() + bookKeepingEntry->DieInterleavedTime, this,

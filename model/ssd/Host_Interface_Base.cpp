	...
	void Input_Stream_Manager_Base::Update_transaction_statistics(NVM_Transaction* transaction)
	{
		switch (transaction->Type)
		{
			case Transaction_Type::READ:
				...
				this->input_streams[transaction->Stream_id]->STAT_sum_of_read_transactions_waiting_time += (MQsim_Simulator->Time() - transaction->Issue_time) - transaction->STAT_execution_time - transaction->STAT_transfer_time;
				break;
			case Transaction_Type::WRITE:
				...
				this->input_streams[transaction->Stream_id]->STAT_sum_of_write_transactions_waiting_time += (MQsim_Simulator->Time() - transaction->Issue_time) - transaction->STAT_execution_time - transaction->STAT_transfer_time;
				break;
			default:
				break;
		}

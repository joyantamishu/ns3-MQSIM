...
namespace Host_Components
{
	...
	void PCIe_Link::Deliver(PCIe_Message* message)
	{
		switch (message->Destination) {
			case PCIe_Destination_Type::HOST://Message from SSD device to the host
				...
				MQsim_Simulator->Register_sim_event(MQsim_Simulator->Time() + estimate_transfer_time(message), this, (void*)(intptr_t)PCIe_Destination_Type::HOST, static_cast<int>(PCIe_Link_Event_Type::DELIVER));
				break;
			case PCIe_Destination_Type::DEVICE://Message from Host to the SSD device
				...
				MQsim_Simulator->Register_sim_event(MQsim_Simulator->Time() + estimate_transfer_time(message), this, (void*)(intptr_t)PCIe_Destination_Type::DEVICE, static_cast<int>(PCIe_Link_Event_Type::DELIVER));
				break;
			default:
				break;
		}
	...
	void PCIe_Link::Execute_MQsim_Simulator_event(MQSimEngine::Sim_Event* event)
	{
		...
		switch (destination) {
			case PCIe_Destination_Type::HOST:
				...
				if (Message_buffer_toward_root_complex.size() > 0) {//There are active transfers
					MQsim_Simulator->Register_sim_event(MQsim_Simulator->Time() + estimate_transfer_time(Message_buffer_toward_root_complex.front()),
			...
			case PCIe_Destination_Type::DEVICE:
				...
				if (Message_buffer_toward_ssd_device.size() > 0) {
					MQsim_Simulator->Register_sim_event(MQsim_Simulator->Time() + estimate_transfer_time(Message_buffer_toward_ssd_device.front()),

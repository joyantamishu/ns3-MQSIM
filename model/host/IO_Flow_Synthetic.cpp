...
#include "ns3/IO_Flow_Synthetic.h"
using namespace ns3;
																														  ...
																														  seed(seed), generate_aligned_addresses(generate_aligned_addresses), alignment_value(alignment_value)
{
	...
	{
		if (stop_time > 0) {
			if (MQsim_Simulator->Time() > stop_time) {
				return NULL;
			}
		...
		request->Arrival_time = MQsim_Simulator->Time();
	...
	void IO_Flow_Synthetic::Start_simulation() 
	{
		...
		if (generator_type == Utils::Request_Generator_Type::BANDWIDTH) {
			MQsim_Simulator->Register_sim_event((sim_time_type)random_time_interval_generator->Exponential((double)Average_inter_arrival_time_nano_sec), this, 0, 0);
		} else {
			MQsim_Simulator->Register_sim_event((sim_time_type)1, this, 0, 0);
		}
	}
	...
	void IO_Flow_Synthetic::Execute_MQsim_Simulator_event(MQSimEngine::Sim_Event* event)
	{
		if (generator_type == Utils::Request_Generator_Type::BANDWIDTH) {
			Host_IO_Request* req = Generate_next_request();
			if (req != NULL) {
				Submit_io_request(req);
				MQsim_Simulator->Register_sim_event(MQsim_Simulator->Time() + (sim_time_type)random_time_interval_generator->Exponential((double)Average_inter_arrival_time_nano_sec), this, 0, 0);
			}
		} else {

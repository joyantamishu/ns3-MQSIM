																												...
																												enabled_logging(enabled_logging), logging_period(logging_period), logging_file_path(logging_file_path)
{
	...
	{
		sim_time_type device_response_time = MQsim_Simulator->Time() - request->Enqueue_time;
		sim_time_type request_delay = MQsim_Simulator->Time() - request->Arrival_time;
		...
		if (stop_time > 0) {
			progress = int(MQsim_Simulator->Time() / (double)stop_time * 100);
		} else {
		...
		if (MQsim_Simulator->Time() > next_logging_milestone) {
			log_file << MQsim_Simulator->Time() / SIM_TIME_TO_MICROSECONDS_COEFF << "\t" << Get_device_response_time_short_term() << "\t" << Get_end_to_end_request_delay_short_term() << std::endl;
			STAT_sum_device_response_time_short_term = 0;
			STAT_sum_request_delay_short_term = 0;
			STAT_serviced_request_count_short_term = 0;
			next_logging_milestone = MQsim_Simulator->Time() + logging_period;
		}
	}
	...
	void IO_Flow_Base::NVMe_consume_io_request(Completion_Queue_Entry* cqe)
	{
		...
		while(waiting_requests.size() > 0) {
			if (!NVME_SQ_FULL(nvme_queue_pair) && available_command_ids.size() > 0) {
				...
				new_req->Enqueue_time = MQsim_Simulator->Time();
		...
		switch (SSD_device_type) {
			case HostInterface_Types::NVME:
				...
				if (NVME_SQ_FULL(nvme_queue_pair) || available_command_ids.size() == 0) {
					...
					request->Enqueue_time = MQsim_Simulator->Time();
					pcie_root_complex->Write_to_device(nvme_queue_pair.Submission_tail_register_address_on_device, nvme_queue_pair.Submission_queue_tail);//Based on NVMe protocol definition, the updated tail pointer should be informed to the device
				}
				break;
			case HostInterface_Types::SATA:
	...
	void IO_Flow_Base::Report_results_in_XML(std::string name_prefix, Utils::XmlWriter& xmlwriter)
	{
		...
		attr = "IOPS";
		val = std::to_string((double)STAT_generated_request_count / (MQsim_Simulator->Time() / SIM_TIME_TO_SECONDS_COEFF));
		xmlwriter.Write_attribute_string(attr, val);

		attr = "IOPS_Read";
		val = std::to_string((double)STAT_generated_read_request_count / (MQsim_Simulator->Time() / SIM_TIME_TO_SECONDS_COEFF));
		xmlwriter.Write_attribute_string(attr, val);

		attr = "IOPS_Write";
		val = std::to_string((double)STAT_generated_write_request_count / (MQsim_Simulator->Time() / SIM_TIME_TO_SECONDS_COEFF));
		xmlwriter.Write_attribute_string(attr, val);
		...
		attr = "Bandwidth";
		val = std::to_string((double)STAT_transferred_bytes_total / (MQsim_Simulator->Time() / SIM_TIME_TO_SECONDS_COEFF));
		xmlwriter.Write_attribute_string(attr, val);

		attr = "Bandwidth_Read";
		val = std::to_string((double)STAT_transferred_bytes_read / (MQsim_Simulator->Time() / SIM_TIME_TO_SECONDS_COEFF));
		xmlwriter.Write_attribute_string(attr, val);

		attr = "Bandwidth_Write";
		val = std::to_string((double)STAT_transferred_bytes_write / (MQsim_Simulator->Time() / SIM_TIME_TO_SECONDS_COEFF));
		xmlwriter.Write_attribute_string(attr, val);

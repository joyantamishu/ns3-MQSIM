...
namespace Host_Components
{
	...
	void SATA_HBA::Execute_MQsim_Simulator_event(MQSimEngine::Sim_Event* event)
	{
		switch ((HBA_Sim_Events)event->Type)
		{
			case HBA_Sim_Events::CONSUME_IO_REQUEST:
			{
				...
				if(consume_requests.size() > 0) {
					MQsim_Simulator->Register_sim_event(MQsim_Simulator->Time() + hba_processing_delay, this, NULL, static_cast<int>(HBA_Sim_Events::CONSUME_IO_REQUEST));
				}
				break;
			}
			case HBA_Sim_Events::SUBMIT_IO_REQUEST:
			{
				...
				if (host_requests.size() > 0) {
					MQsim_Simulator->Register_sim_event(MQsim_Simulator->Time() + hba_processing_delay, this, NULL, static_cast<int>(HBA_Sim_Events::SUBMIT_IO_REQUEST));
				}

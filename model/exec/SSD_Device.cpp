...
#include "ns3/Flash_Block_Manager.h"
#include "ns3/Data_Cache_Manager_Flash_Advanced.h"
#include "ns3/Data_Cache_Manager_Flash_Simple.h"
#include "ns3/Address_Mapping_Unit_Base.h"
#include "ns3/Address_Mapping_Unit_Page_Level.h"
#include "ns3/Address_Mapping_Unit_Hybrid.h"
#include "ns3/GC_and_WL_Unit_Page_Level.h"
#include "ns3/TSU_OutofOrder.h"
#include "ns3/TSU_Priority_OutOfOrder.h"
#include "ns3/TSU_FLIN.h"
#include "ns3/ONFI_Channel_NVDDR2.h"
...
SSD_Device *SSD_Device::my_instance; //Used in static functions
	...
	SSD_Device::SSD_Device(Device_Parameter_Set *parameters, std::vector<IO_Flow_Parameter_Set *> *io_flows) : MQSimEngine::Sim_Object("SSDDevice")
	{
		...
		MQsim_Simulator->AddObject(device);
		...
		case NVM::NVM_Type::FLASH:
		{
			...
			case SSD_Components::ONFI_Protocol::NVDDR2:
			{
				...
				for (unsigned int channel_cntr = 0; channel_cntr < parameters->Flash_Channel_Count; channel_cntr++)
				{
					...
					for (unsigned int chip_cntr = 0; chip_cntr < parameters->Chip_No_Per_Channel; chip_cntr++)
					{
						...
						MQsim_Simulator->AddObject(chips[chip_cntr]); //Each simulation object (a child of MQSimEngine::Sim_Object) should be added to the engine
					}
					...
					device->Channels.push_back(channels[channel_cntr]); //Channels should not be added to the MQsim_Simulator core, they are passive object that do not handle any simulation event
				}
				...
				MQsim_Simulator->AddObject(device->PHY);
				break;
			}
			default:
			...
			gcwl = new SSD_Components::GC_and_WL_Unit_Page_Level(ftl->ID() + ".GCandWLUnit", amu, fbm, tsu, (SSD_Components::NVM_PHY_ONFI *)device->PHY,
																 ...
																 parameters->Seed++);
			MQsim_Simulator->AddObject(gcwl);
			...
			MQsim_Simulator->AddObject(device->Host_interface);
	...
	void SSD_Device::Execute_MQsim_Simulator_event(MQSimEngine::Sim_Event *event)
	{
	}

...
#include "ns3/IO_Flow_Trace_Based.h"
...
namespace ns3
{
	...
	{
		MQsim_Simulator->AddObject(this);
		...
		MQsim_Simulator->AddObject(this->Link);
		...
		for (uint16_t flow_id = 0; flow_id < parameters->IO_Flow_Definitions.size(); flow_id++) {
			...
			MQsim_Simulator->AddObject(io_flow);
		}
	...
	void Host_System::Execute_MQsim_Simulator_event(MQSimEngine::Sim_Event* event)
	{
	}

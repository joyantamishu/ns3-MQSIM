...
namespace NVM
{
	namespace FlashMemory
	{
		...
		public:
			...
			void StartCMDXfer()
			{
				this->lastTransferStart = MQsim_Simulator->Time();
			}
			...
			{
				this->STAT_totalXferTime += (MQsim_Simulator->Time() - this->lastTransferStart);
				if (this->idleDieNo != die_no)
					STAT_totalOverlappedXferExecTime += (MQsim_Simulator->Time() - lastTransferStart);
				this->Dies[command->Address[0].DieID]->STAT_TotalXferTime += (MQsim_Simulator->Time() - lastTransferStart);
			...
			void Setup_triggers();
			void Execute_MQsim_Simulator_event(MQSimEngine::Sim_Event*);

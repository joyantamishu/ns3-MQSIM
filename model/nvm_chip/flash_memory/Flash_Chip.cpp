...
namespace NVM
{
	namespace FlashMemory
	{
		...
		void Flash_Chip::Execute_MQsim_Simulator_event(MQSimEngine::Sim_Event* ev)
		{
		...
		void Flash_Chip::start_command_execution(Flash_Command* command)
		{
			...
			targetDie->Expected_finish_time = MQsim_Simulator->Time() + Get_command_execution_latency(command->CommandCode, command->Address[0].PageID);
			targetDie->CommandFinishEvent = MQsim_Simulator->Register_sim_event(targetDie->Expected_finish_time,
			...
			if (status == Internal_Status::IDLE) {
				executionStartTime = MQsim_Simulator->Time();
		...
		void Flash_Chip::finish_command_execution(Flash_Command* command)
		{
			...
			if (idleDieNo == die_no) {
				...
				STAT_totalExecTime += MQsim_Simulator->Time() - executionStartTime;
				if (this->lastTransferStart != INVALID_TIME) {
					STAT_totalOverlappedXferExecTime += MQsim_Simulator->Time() - lastTransferStart;
				}
			}
		...
		void Flash_Chip::Suspend(flash_die_ID_type dieID)
		{
			...
			targetDie->RemainingSuspendedExecTime = targetDie->Expected_finish_time - MQsim_Simulator->Time();
			MQsim_Simulator->Ignore_sim_event(targetDie->CommandFinishEvent);//The MQsim_Simulator engine should not execute the finish event for the suspended command
			targetDie->CommandFinishEvent = NULL;
		...
		void Flash_Chip::Resume(flash_die_ID_type dieID)
		{
			...
			targetDie->Expected_finish_time = MQsim_Simulator->Time() + targetDie->RemainingSuspendedExecTime;
			targetDie->CommandFinishEvent = MQsim_Simulator->Register_sim_event(targetDie->Expected_finish_time,
		...
		void Flash_Chip::Report_results_in_XML(std::string name_prefix, Utils::XmlWriter& xmlwriter)
		{
			...
			val = std::to_string(STAT_totalExecTime / double(MQsim_Simulator->Time()));
			xmlwriter.Write_attribute_string_inline(attr, val);

			attr = "Fraction_of_Time_in_DataXfer";
			val = std::to_string(STAT_totalXferTime / double(MQsim_Simulator->Time()));
			xmlwriter.Write_attribute_string_inline(attr, val);

			attr = "Fraction_of_Time_in_DataXfer_and_Execution";
			val = std::to_string(STAT_totalOverlappedXferExecTime / double(MQsim_Simulator->Time()));
			xmlwriter.Write_attribute_string_inline(attr, val);

			attr = "Fraction_of_Time_Idle";
			val = std::to_string((MQsim_Simulator->Time() - STAT_totalOverlappedXferExecTime - STAT_totalXferTime) / double(MQsim_Simulator->Time()));
			xmlwriter.Write_attribute_string_inline(attr, val);
